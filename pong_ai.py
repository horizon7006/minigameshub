...
class PongGameAI:
    ...
    BALL_SIZE = 16
    SPEED = 5
    AI_SPEED = 4  # how fast the AI paddle can move
    ...
    def ai_move(self):
        # simple AI: move paddle toward ball's y
        paddle_center = self.right_paddle[1] + self.PADDLE_H/2
        if paddle_center < self.ball[1] and self.right_paddle[1] + self.PADDLE_H < self.HEIGHT:
            self.right_paddle[1] += self.AI_SPEED
        elif paddle_center > self.ball[1] and self.right_paddle[1] > 0:
            self.right_paddle[1] -= self.AI_SPEED
    ...
    def run(self):
        ...
        while True:
            ...
            self.handle_input()
            self.ai_move()       # <— AI moves right paddle each frame
            self.update_ball()
            self.draw()
            self.clock.tick(60)
